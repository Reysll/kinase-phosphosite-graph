"""
LOO evaluation on the control-specific (healthy sample) network.

Correlation edges: site_corr_ctrl_pos / site_corr_ctrl_neg
These are built from Pearson correlations of raw phosphosite abundances
across the 18 healthy/control samples in the liver proteomics dataset.
...
from src_prediction.relation_filters import GENERIC_BASE_RELATIONS, SITE_CORR_CTRL_RELATIONS


def main() -> None:
    experiment_name = "control_multi_kinase"
    ...
    print("=== Loading liver graph (control-specific correlation variant) ===")
    ...
    # Control model: base relations + healthy-sample correlation edges only.
    ...
    print("=== Running leave-one-out on multi-kinase sites: control (healthy) model ===")
