...
INPUT_PATH = Path("outputs_prediction/frozen_trials_multi_kinase_sites.csv.gz")
OUTPUT_PATH = Path("outputs_prediction/frozen_trials_multi_kinase_sites_10.csv.gz")
...
def main() -> None:
    ...
    if "site_node_id" not in df.columns:
        raise ValueError("Expected column 'site_node_id' was not found in the input file.")

    # Keep one row per site so the 10 trials cover 10 different sites
    df10 = df.drop_duplicates(subset=["site_node_id"]).head(10).copy()

    # Renumber trial_index from 1 to 10
    if "trial_index" in df10.columns:
        df10["trial_index"] = range(1, len(df10) + 1)
