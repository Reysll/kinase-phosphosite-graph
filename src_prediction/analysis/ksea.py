...
def compute_ksea_scores(
    ...
    sqrt(m) is in the numerator: more substrates increase confidence in the
    enrichment estimate (lower SEM), so larger substrate sets are rewarded,
    not penalized.
    ...
    for kinase_id, substrates in substrate_sets.items():
        ...
        # Wiredja 2017: sqrt(m) in the numerator, divided by global SD
        score = (mean_sub - global_mean) * np.sqrt(n) / global_sd
