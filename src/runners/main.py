...
from src.graph.preprocess import preprocess_kinase_substrate, preprocess_ppi, preprocess_ptmcode2
from src.graph.sanity_checks import check_site_collisions
...
def _add_protein_metadata(
    ...
    out.loc[protein_mask, "is_phosphatase"] = (
        protein_ids.isin(phosphatase_protein_ids).astype(int).values
    )
...
def main() -> None:
    ...
    kinase_protein_ids = set(k_edges.loc[k_edges["relation"] == "phosphorylates", "source"])
    ...
    phosphatase_protein_ids = set(p_edges.loc[p_edges["relation"] == "dephosphorylates", "source"])
